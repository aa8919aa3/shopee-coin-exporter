...
本次新增「短影音」、「每日登入」、「評價」、「VIP訂閱」、「蝦皮聯名卡」與「蝦幣獎勵」六個活動分類，並將分類選單改為由單一共用清單動態建立，避免解析規則與 UI 選項日後不同步。
...
在穩定量測區間內，10,000 筆資料的統計約需 3.77 ms、關鍵字篩選約 2.11 ms、CSV 匯出約 37.76 ms、TXT 匯出約 27.32 ms。100,000 筆資料的統計約需 35.19 ms、關鍵字篩選約 14.54 ms、CSV 匯出約 202.25 ms、TXT 匯出約 150.81 ms。這些本機運算不會成為上萬筆資料的主要等待來源；實際總抓取時間仍主要由蝦皮 API 的循序網路請求決定。
...
分類清單目前共有 15 個可選項，包含上述六類與每日簽到、購物／訂單、消費折抵、蝦幣過期、退款／補償、蝦皮遊戲、蝦皮直播、行銷活動／任務及其他活動。相關回歸測試會驗證每個新分類至少一個實際型態的標題。
